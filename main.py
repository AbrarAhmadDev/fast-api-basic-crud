...
async def read_campaigns(request : Request, session: SessionDep, page: int = Query(1,ge=1), page_size : int = Query(3,ge=1)): # type: ignore
    limit = page_size
    offset = (page-1)*limit
...
@app.post("/campaigns", status_code=201,response_model=Response[CampaignCreate])
